...
def exposure_fusion(sequence, alphas=(1.0, 1.0, 1.0), best_illumination=0.5, sigma=0.2, eps=1e-12,
    ...
    for s in range(S):
        ...
        # 求亮度
        illumination = [numpy.exp(-0.5 * mse(sequence[s][:, :, ch], best_illumination) / (sigma * sigma)) for ch in
                        range(3)]
        illumination = numpy.prod(illumination, axis=0)
        # 三者加权
        cur_weight = numpy.power(contrast, alphas[0]) * numpy.power(saturation, alphas[1]) * numpy.power(illumination,
                                                                                                         alphas[2])
        weights.append(cur_weight)
    # 归一化
    normalize = lambda x: x / numpy.sum(x, axis=0, keepdims=True)
    ...
    if (use_gaussi == True):
        ...
        smoothed_fusion = numpy.sum(
            numpy.stack([smoothed_weights, smoothed_weights, smoothed_weights], axis=-1) * sequence, axis=0)
    ...
    if (use_lappyr == True):
        ...
        fused_laplace_pyramid = [numpy.sum([sequence_laplace_pyramids[k][n] *
                                            numpy.atleast_3d(sequence_weights_pyramids[k][n]) for k in range(S)],
                                           axis=0) for n in range(layers_num)]
...
images_dir = r"C:\Users\DELL\PycharmProjects\process\tmp\Img"
save_dir = r"C:\Users\DELL\PycharmProjects\process\tmp\Img"
